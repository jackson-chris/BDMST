...
Edge* Graph::findEdge(Vertex* a, Vertex* b){
	Edge* edgeWalkPtr;
	vector<Edge*>::iterator e;
	for ( e = a->edges.begin(); e < a->edges.end(); e++ ) {
		edgeWalkPtr = *e;
		if(edgeWalkPtr->getSource(NULL)->data == b->data){
			return edgeWalkPtr;
		} else if(edgeWalkPtr->getDestination(NULL)->data == b->data){
			return edgeWalkPtr;
		}
	}
	exit(1);
 ...
 */
int Graph::insertEdge(int fromKey, int toKey, double weight, double pLevelIn) {
    Edge *newPtr;
