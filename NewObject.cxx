...
void prim(Graph* gHub, vector<Edge*> tree);
...
void prim(Graph* gHub, vector<Edge*> tree) {
    ...
    Vertex* current = gHub->getFirst();
    Hub* u;
    Edge *pEdge, *e;
    vector<Edge*>::iterator ie;
    do{
    	heap->insert(current->pHub);
    	dist[current->data] = std::numeric_limits<double>::infinity();
    	parent[current->data] = NULL;
    	current = current->pNextVert;
    }while(current->pNextVert != NULL);
    
    while (!heap->isEmpty()){
    	u = heap->deleteMax();
    	if(parent[u->vertId] != NULL){
    		e = gHub->findEdge(u->vert, parent[u->vertId]->vert);
    		tree.push_back(e);
    	}
    	for ( ie = u->vert->edges.begin() ; ie < u->vert->edges.end(); ie++ ) {
    		pEdge = *ie;
    		if(heap->find(pEdge->getSource(NULL)) && pEdge->pLevel > dist[pEdge->getSource(NULL)->data]){
    			dist[pEdge->getSource(NULL)->data] = pEdge->pLevel;
    			parent[pEdge->getSource(NULL)->data] = u;
    		}
    		if(heap->find(pEdge->getDestination(NULL)) && pEdge->pLevel > dist[pEdge->getDestination(NULL)->data]){
    			dist[pEdge->getDestination(NULL)->data] = pEdge->pLevel;
    			parent[pEdge->getDestination(NULL)->data] = u;
    		}
    	}
   }
}
