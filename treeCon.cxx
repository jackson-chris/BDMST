...
void compute(Graph* g, int d, processFile p, int i) {
    ...
    vector<Edge*> best = treeConstruct(g, d);
    for ( ed = best.begin(); ed < best.end(); ed++ ) {
        edgeWalkPtr = *ed;
        treeCost+=edgeWalkPtr->weight;
    }
    cout << "Cost of best: " << treeCost << endl;
...
*
*/
bool asc_cmp_weight(Edge *a, Edge *b) {
...
void printEdge(Edge* e) {
    cout << "RESULT: ";
    cout << e->a->data << " " << e->b->data << " " << e->weight << endl;
}
...
vector<Edge*> treeConstruct(Graph *g, int d) {
    ...
    populateVector(g, &v);
    //  Sort edges in decending order based upon weight level
    sort(v.begin(), v.end(), des_cmp_cost);
    //  Select 5n edges from the end of v( the lowest cost edges) and put them into c.
    ...
    for(unsigned int index = 1; index <= g->getNumNodes(); index++) {
        hubs.push_back(new Hub());
        hubs[index]->vertId = index;
    ...
    for(ihubs1 = hubs.begin() + 1; ihubs1 < hubs.end(); ihubs1++) {
        h = *ihubs1;
        if(h->inTree == false) {
            h->inTree = true;
    ...
    for(unsigned int i = 0; i < treeHubs->size(); i++) {
        ...
        for(unsigned int j = i+1; j < treeHubs->size(); j++) {
            ...
            for(iedge3 = v1->edges.begin(); iedge3 < v1->edges.end(); iedge3++) {
                ...
                if(pEdge->b->data == v2->data && (*hubs)[pEdge->b->data]->inTree == true && (*hubs)[v2->data]->inTree == true)
...
void getHubs(Graph* g, BinaryHeap* heap, vector<Edge*> *v, vector<Edge*> *c, vector<Edge*> *tree, vector<Hub*> *hubs, vector<Hub*> *treeHubs, const unsigned int & MAX_TREE_SIZE, const unsigned int & CAN_SIZE) {
    ...
    //  Logic
    while(treeCount != 1 * MAX_TREE_SIZE && !isEmpty) {
        if(heap->topSize() != 0) {
    ...
    while(!done) {
        ...
        while(!back) {
            ...
            if(pVertChk->data == first){
                //cout << "If pVertCheck = first\n";
                //cout << pVertChk->data << endl;;
                back = true;
            }
            //cout << pVertChk->pNextVert->data << endl;
            //cout << pVertChk->data << endl;;
        }
        if(pEdgeMin) {
...
void heapifyHubs(vector<Hub*> *hubs, vector<Edge*> *c, int & numEdges, BinaryHeap* heap) {
    ...
    for(ihubs2 = hubs->begin() + 1; ihubs2 < hubs->end(); ihubs2++) {
        pHub = *ihubs2;
